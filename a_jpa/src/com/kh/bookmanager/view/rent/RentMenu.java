		...
		do {
			...
			case 4: //대출건을 조회할 사용자의 아이디를 입력받아
					...
					//대출건 목록을 출력한 다음
				 	...
				 	userId = sc.next();
				 	List<Rent> rentList = rentController.findAllRentByUserId(userId);
					rentList.stream().forEach(e -> System.out.println(e));
					
					//사용자에게 대출건 상세 조회여부를 물어
				    //사용자가 대출건 상세 조회를 하겠다고 하면
					System.out.print("상세 조회할 대출건이 있습니까?(y/n) : ");
					if(sc.next().equalsIgnoreCase("y")) {
						//상세 조회할 대출건 번호를 입력받고
					    //해당 대출건의 대출도서 목록을 출력
						//출력 내용 : rmIdx, rbIdx, bIdx, 도서명, 반납일자, 반납여부
						System.out.print("상세 조회할 대출번호를 입력 : ");
